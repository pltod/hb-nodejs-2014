...
* This repository contains the solution to the original assignment specified at https://github.com/asotirov/that-node-task
...
> ```npm test``` - run the mocha test suite without the need of having it globally installed


* Development mode

> ```npm run start-dev``` - starts the application via nodemon. Used while fixing all the tests to have automatic server restarting on each change.
